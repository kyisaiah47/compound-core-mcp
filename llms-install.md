...
The server runs from npm via stdio; the only configuration is the `PARSERAIL_API_KEY` environment variable.
...
```json
{
  "mcpServers": {
    "parserail": {
      ...
      "env": {
        "PARSERAIL_API_KEY": "ksk_live_your_key_here"
      }
    }
  }
}
...
- **Claude Code**: `claude mcp add parserail -e PARSERAIL_API_KEY=ksk_live_… -- npx -y parserail-mcp`
- **Claude (custom connector, no install)**: use the hosted URL `https://parserail.thecompound.tech/mcp` with your key.
- **Cursor / VS Code**: same JSON under their MCP settings; the server also lives in the official MCP registry as `tech.thecompound/parserail-mcp`.
